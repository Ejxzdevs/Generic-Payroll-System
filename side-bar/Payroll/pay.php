...
$pay_ind_emp = "SELECT * FROM (SELECT Employee_Types,Employees_ID,@did:= Department_ID,
 ...
 FROM tbl_employees_information WHERE Employees_ID = $id)tbl_employees_information INNER JOIN (SELECT * FROM tbl_personal_information WHERE Employees_ID = $id)tbl_personal_information ON tbl_employees_information.Employees_ID = tbl_personal_information.Employees_ID INNER JOIN (SELECT *,SUM(Daily_Salary) as total_salary,SUM(Regular_Salary) as Basic_Pay,SUM(Ot_Salary) as overtime_pay, SUM(Late_D) as total_late_deduction,SUM(Undertime_D) as total_undertime_deduction FROM tbl_salary_earning WHERE Earning_Date BETWEEN '$start' AND '$end'AND Employees_ID = $id )tbl_salary_earning ON tbl_personal_information.Employees_ID = tbl_salary_earning.Employees_ID 
...
<body>
	<div class="payroll-container">
		<div class="pay_header">
			<div class="pay-right">
					<a style="background: green" href="download_payslip.php?id=<?php echo$id; ?>&payroll_id=<?php echo $payroll_id; ?>" class="print">
					Download
				</a>
				<a href="print_ind_payslip.php?id=<?php echo$id; ?>&payroll_id=<?php echo $payroll_id; ?>" class="print">
					PRINT
				</a>
				<a href="prepare_payroll_layout.php?payroll_id=<?php echo $payroll_id;  ?>" class="cancel">
					BACK
				</a>
			</div>
			<div class="pay-left">
