...
$select_all_id = "SELECT * FROM (SELECT * FROM tbl_employees_information WHERE Employee_Types = '$emp_type') tbl_employees_information INNER JOIN (SELECT * FROM tbl_time_entries WHERE Date_Attendance BETWEEN '$start' AND '$end' GROUP BY Employees_ID) tbl_time_entries on tbl_employees_information.Employees_ID = tbl_time_entries.Employees_ID";
...
$total_tardiness = $result_pay_ind_emp['total_late_deduction'] + $result_pay_ind_emp['total_undertime_deduction'];
...
// total_tax pay
		...
		$final_deduction = $total_tax_d + $total_d + $total_tardiness;
...
$pdf->SetFont('Arial', '', 10);
$pdf->cell(95,10,"  Tardiness "  ,'L',0,'C');
$pdf->cell(95,10,"".$total_tardiness ,'LR',1,'C');
...
$earning_id = $result_pay_ind_emp['Earning_ID'];


$insert_salary_record = "INSERT INTO `tbl_salary_record`(`Employees_ID`, `Total_Deductions`, `Total_Tax`,`Total_Allowances`, `Gross_Salary`, `Net_Salary`, `Date_From`, `Date_To`, `Date_Paid`,`Payroll_ID`,`Earning_ID`) VALUES ($id,$total_d,$total_tax_d,$total_a,$gross_salary,$net_salary,'$start','$end','$date_pay',$payroll_id,$earning_id)";
$query = mysqli_query($conn,$insert_salary_record);

}WHILE($result_id=$query_id->fetch_assoc());

$filename = "All_Payslip/payslip.pdf";
...
$udpate_paid = mysqli_query($conn,"UPDATE tbl_payroll_list SET `Payroll_Status`= 'Paid' WHERE Payroll_ID = $payroll_id");

$udpate_emp_paid = mysqli_query($conn,"UPDATE `tbl_salary_earning` SET `Salary_Status`= 'Paid' WHERE Earning_Date BETWEEN '$start' AND '$end'");
