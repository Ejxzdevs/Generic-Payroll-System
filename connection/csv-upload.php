	...
	if($_FILES['file_upload']['size'] > 0)
	{
		...
		while(($getData = fgetcsv($file, 10000, ",")) !== FALSE) {
			...
			if(substr($getData[0],0,3)=="Att"){
        
        	continue;
    		}
    		if(substr($getData[0],0,3)=="Com"){
        
        	continue;
    		}
    		if(substr($getData[0],0,3)=="Wor"){
        
        	continue;
    		}
    		if(substr($getData[0],0,3)=="Dai"){
        
        	continue;
    		}
 
 	 	 	 if(substr($getData[0],0,3)=="Dev"){
        
        	continue;
    		}
    		if(substr($getData[0],7,9)=="ID"){
        
        	continue;
    		}
    		if(substr($getData[0],0,2)=="ID"){
        	substr($getData[0],3,3);
        	echo $start = substr($getData[13],5,11);
        	echo "<br>";
        	echo $end = substr($getData[13],17,18);
        	ECHO $id = substr($getData[0],3,3);
        ...
        if($getData[0]=="Date"){
            ...
            // check if the date if exist
            echo $date = date("Y-m-d",strtotime($getData[0]));
            echo "<BR>";   
            echo $in = $getData[2]." ";
            // echo $out = $getData[3]." ";
                if($getData[3] == ''){
            ...
            $select_daily_rate = "SELECT * FROM 
                ...
                // echo "<br>";
                // echo "<br>";
        echo    $regular_salary = ((((((($result_daily_rate['Daily_Rate']/8)/60)/100)*$sched_rate)/100)*$percent_holiday)*$total_regular_time);

        echo    $Ot_salary = ((((((((($result_daily_rate['Daily_Rate']/8)/60)/100)*$sched_rate)/100)*$overtime_rate)/100)*$percent_holiday)*$min_overtime);

        echo  $late_deduction = ((((((($result_daily_rate['Daily_Rate']/8)/60)/100)*$sched_rate)/100)*$percent_holiday)*$min_late);

        echo    $undertime_deduction = ((((((($result_daily_rate['Daily_Rate']/8)/60)/100)*$sched_rate)/100)*$percent_holiday)*$min_undertime);

         echo   $total_salary = $regular_salary + $Ot_salary;
         ...
         $update_csv = "UPDATE `tbl_upload_csv` SET `Start_Date`='$start',`End_Date`='$end' WHERE Date_Uploaded = '$current_date' ";
